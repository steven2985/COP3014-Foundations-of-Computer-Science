...
Description:       Program that process the customer purchase orders for a nursery. Calculate tax, discount and total cost of the purchase
                   then print the results on a file.
*/
...
void output(ofstream& out, const orderRecord customerOrder)
{
	...
	out << setw(10) << left << customerOrder.cName;

	out << setw(7) << right << customerOrder.plantCost;

	out << setw(6) << right << noshowpoint << customerOrder.quantity;

	cout.precision(2);
	out << setw(8) << right << showpoint << customerOrder.netCost;
	...
	out << setw(7) << right << customerOrder.discount;
	out << setw(9) << right << customerOrder.totalCost << endl;
